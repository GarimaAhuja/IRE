...
map<string, map<string,float> > entitySim;
map<string, map<string,float> > verbSim;

float getEntitySim(vector<string> v1,vector<string> v2)
{
	float sim=0;
...
float getVerbSim(vector<string> v1,vector<string> v2)
{
	float sim=0;
	for(int i=0;i<v1.size();i++)
	{
...
void getSims()
{
	ifstream verbFile("verb_similarity.txt");
	...
	while(line.size()>0)
	{
		...
		verbSim.insert(std::pair<string,map<string,float> >(curr,empty));
	...
	while(getline(verbFile,line))
	{
		...
		while(line.size()>0)
		{
			...
			for(map<string,map<string,float> >::iterator it=verbSim.begin();it!=verbSim.end();it++)
			{
				it->second.insert(std::pair<string,float>(word,atof(curr.c_str())));
			}
	...
	ifstream entityFile("entity_similarity.txt");
	...
	while(line.size()>0)
	{
		string curr=line.substr(0,line.find('\t'));
		map<string,float> empty;
		entitySim.insert(std::pair<string,map<string,float> >(curr,empty));
		line=line.substr(line.find('\t')+1);
	}
	while(getline(entityFile,line))
	{
		...
		while(line.size()>0)
		{
			...
			for(map<string,map<string,float> >::iterator it=entitySim.begin();it!=entitySim.end();it++)
			{
				it->second.insert(std::pair<string,float>(word,atof(curr.c_str())));
			}
		...
		for(int i=0;i<5;i++)
		{
			...
			for(int j=0;j<tweets.size();j++)
			{
				...
				for(int k=0;k<entities[j].size();k++)
				{
					...
					{
						sim=0.7*getEntitySim(clusterE[entityToClusters[entities[j][k]][l]],entities[j])+0.3*getVerbSim(clusterV[entityToClusters[entities[j][k]][l]],verbs[j]);
						if(maxSim<sim)
						{
							maxSim=sim;
